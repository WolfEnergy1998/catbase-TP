<template>
  ...
  <div class="flex justify-center">
    <p class="q-mt-md">
      {{ $t('catNames1') }} <b>{{ typeof pagination.rowsNumber === 'number'? pagination.rowsNumber.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' '): pagination.rowsNumber }}</b>
      {{ $t('catNames2') }} <b>{{ selectedCharacter }}</b> {{$t('catNames3')}} <b>{{ findData.sex=='F'?'Dam':'Sire' }} :)</b>
    </p>
  </div>
  <q-table
    ref="tableRef"
